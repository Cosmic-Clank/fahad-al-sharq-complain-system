...
import ComplaintCard from "./components/ComplaintCard"; // The Client Component to display the complaint
...
async function ComplaintDetailPage({ params }: ComplaintDetailPageProps) {
	const complaintId = Number(params.slug);
	...
	const formattedComplaint = {
		...
		createdAt: complaint.createdAt.toISOString(), // Convert Date to ISO string
		...
		responses: complaint.responses.map((resp) => ({
			...
			createdAt: resp.createdAt instanceof Date ? resp.createdAt.toISOString() : String(resp.createdAt),
			responder: {
